...
public class BooksController {
    ...
    @PutMapping(path = "{bookID}", consumes = MediaType.APPLICATION_JSON_UTF8_VALUE, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    public BookDTO put(@PathVariable long bookID, @RequestBody BookDTO bookDTO) {

        Book aBook = new Book(bookDTO.id, bookDTO.title, bookDTO.author, bookDTO.description, bookDTO.price, bookDTO.published);
        bookService.update(aBook);

        return new BookDTO(aBook);
    }
